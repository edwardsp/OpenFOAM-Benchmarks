...
for ppn in $ppn_list; do
    ...
    for nodes in $node_list; do
        ...
        decompose_id=$(qsub -N "decompose_${name}_${nodes}x${ppn}" -l select=1:ncpus=${ncpus}:mpiprocs=$(( $ncpus / 2 )),place=scatter:excl -W depend=afterok:${mesh_id} -- $(pwd)/submit_decompose.sh $name $nodes $ppn | cut -d'.' -f1)
